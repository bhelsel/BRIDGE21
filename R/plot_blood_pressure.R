...
generate_bp_plot <- function(bpsys, bpdia){
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::annotation_custom(grid::rasterGrob(img, width = 0.5, height = 0.25, x = 0.3, y = 0.15)) +
    round_rectangle(x = 0.75, y = 0.5, width = 0.35, height = 0.97, radius = 0.2,
                    fill = grid::linearGradient(c("gray90", "black"), x1 = 0, y1 = 0, x2 = 1, y2 = 1)) +
    round_rectangle(x = 0.75, y = 0.5, width = 0.32, height = 0.93, radius = 0.2, fill = "white") +
    # Systolic
    round_rectangle(x = 0.67, y = 0.18, width = 0.075, height = 0.2, radius = 0.5, fill = bptab$colors[[1]]) +
    round_rectangle(x = 0.67, y = 0.78, width = 0.075, height = 0.2, radius = 0.5, fill = bptab$colors[[4]]) +
    ggplot2::geom_rect(ggplot2::aes(xmin = 64.6, xmax = 72.8, ymin = 60, ymax = bptab$systolic[[1]]), fill = bptab$colors[[1]]) +
    ggplot2::geom_rect(ggplot2::aes(xmin = 64.6, xmax = 72.8, ymin = bptab$systolic[[1]], ymax = bptab$systolic[[2]]), fill = bptab$colors[[2]]) +
    ggplot2::geom_rect(ggplot2::aes(xmin = 64.6, xmax = 72.8, ymin = bptab$systolic[[2]], ymax = bptab$systolic[[3]]), fill = bptab$colors[[3]]) +
    ggplot2::geom_rect(ggplot2::aes(xmin = 64.6, xmax = 72.8, ymin = bptab$systolic[[3]], ymax = bptab$systolic[[4]]), fill = bptab$colors[[4]]) +
    ggplot2::annotate("text", x = 68.5, y = 212, label = "SYS\n(mmHg)", fontface = "bold") +
    ggplot2::annotate("segment", x = 64.6, xend = 72.8, y = bpsys, yend = bpsys, linetype = "dashed") +
    ggplot2::geom_label(ggplot2::aes(x = 68.5, y = bpsys + 7), label = bpsys, fontface = "bold", fill = "white") +
    # Diastolic
    round_rectangle(x = 0.84, y = 0.78, width = 0.075, height = 0.2, radius = 0.5, fill = bptab$colors[[4]]) +
    round_rectangle(x = 0.84, y = 0.18, width = 0.075, height = 0.2, radius = 0.5, fill = bptab$colors[[1]]) +
    ggplot2::geom_rect(ggplot2::aes(xmin = 83.27, xmax = 91.5, ymin = bptab$diastolic[[1]], ymax = bptab$diastolic[[2]]), fill = bptab$colors[[2]]) +
    ggplot2::geom_rect(ggplot2::aes(xmin = 83.27, xmax = 91.5, ymin = bptab$diastolic[[2]], ymax = bptab$diastolic[[3]]), fill = bptab$colors[[3]]) +
    ggplot2::geom_rect(ggplot2::aes(xmin = 83.27, xmax = 91.5, ymin = bptab$diastolic[[3]], ymax = bptab$diastolic[[4]]), fill = bptab$colors[[4]]) +
    ggplot2::annotate("text", x = 87.5, y = 212, label = "DIA\n(mmHg)", fontface = "bold") +
    ggplot2::annotate("segment", x = 83.27, xend = 91.5, y = bpdia, yend = bpdia, linetype = "dashed") +
    ggplot2::geom_label(ggplot2::aes(x = 87.5, y = bpdia + 7), label = bpdia, fontface = "bold") +
    # Numeric Axis Labels
