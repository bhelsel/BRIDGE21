...
plot_time_series_by_date <- function(IMP){
  ...
  data2plot <-  
    ...
    dplyr::ungroup() |>
    dplyr::mutate(category = cut(
      ENMO*1000, breaks = c(-Inf, 44.8, 100.6, 428.8, Inf),
      labels = c("Inactivity", "Light", "Moderate", "Vigorous"))) |>
    dplyr::mutate(timestamp = as.POSIXct(timestamp, "%Y-%m-%d %H:%M", tz = "UTC")) |>
  ...
  plot <- 
    ggplot2::ggplot(data2plot) +
    ggplot2::geom_line(ggplot2::aes(
      x = timestamp, 
      y = ENMO, color = category, group = 1)) +
    ggplot2::facet_wrap(
      ggplot2::vars(
        factor(format(as.Date(timestamp), "%A, %B %d, %Y"),
               levels = date_labels)), 
      nrow = 4, scales = "free_x") +
    ggplot2::scale_x_datetime(
      labels = scales::date_format("%k"),
      breaks = "2 hour", expand = c(0,0)
    ) +
    ggplot2::scale_color_manual(values = colors[1:n_colors],
