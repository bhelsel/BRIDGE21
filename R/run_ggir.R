...
run_ggir <- function(id, acceldir, outputdir){
  ...
  files <- list.files(acceldir, pattern = ".gt3x$", recursive = TRUE, full.names = TRUE)
  ...
  if(!dirname(acceldir) == file.path(outputdir, id)){
    if(!dir.exists(acceldata)) dir.create(acceldata, recursive = TRUE)
    if(!dir.exists(accelres)) dir.create(accelres, recursive = TRUE)
