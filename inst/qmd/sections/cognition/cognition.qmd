...
if (
  ...
  params$kuadrc_mcrt <- params$bold_mcrt <- params$abcds_mcrt <- params$trcds_mcrt <- FALSE
}

# National Task Group - Early Detection and Screen for Dementia (NTG-EDSD)
if (params$kuadrc_ntgedsd | params$bold_ntgedsd | params$id == "test_jayhawk") {
...
if (
  all(apply(ntgedsd_df[setdiff(colnames(ntgedsd_df), date)], 2, is.na)) |
    all(ntgedsd_df[setdiff(colnames(ntgedsd_df), date)] == 0, na.rm = TRUE) |
    nrow(ntgedsd_df) == 0
) {
...
if (params$bold_cognition & params$id != "test_jayhawk") {
