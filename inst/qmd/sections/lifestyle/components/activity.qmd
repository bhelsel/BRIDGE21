...
BRIDGE21:::format_quarto(
  "sections/lifestyle/latex/activity.tex",
  type = "input"
)
...
lastFileName <- tail(unique(accel_df$filename), 1)
accel_df_subset <- accel_df[accel_df$filename == lastFileName, ]
...
accel_plot <- BRIDGE21::plot_physical_activity(
  file.path(accelMetaDir, "ms2.out", sprintf("%s.RData", lastFileName)),
  summary_data = accel_df_subset
)

accelerometer_file_name <- sprintf("%s.png", gsub(".gt3x$", "", lastFileName))
