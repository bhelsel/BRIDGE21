...
dxa <- dxa %>%
  dplyr::mutate(
    dplyr::across(lean:fat, ~ as.numeric(.x) / 1000),
    dplyr::across(mass:fat, ~ round(as.numeric(.x) * 2.205, 1))
  )

sex <- ifelse(
  params$kuadrc_dxa,
  visit$sex,
  intake[!is.na(intake$sex), "sex", drop = TRUE]
)

body_fat_percent_plot <- BRIDGE21::plot_body_fat(
  total_fat = tail(na.omit(dxa$total_fat), 1),
  subject_sex = sex,
  subject_birthdate = dob,
  fittest_date = tail(dxa$Date, 1)
)
