...
#| fig-align: center
#| ft.align: center
#| eval: !expr (params$kuadrc_bmi) | (params$bold_bmi) | (params$abcds_bmi) | (params$trcds_bmi)

anthro_units <- ifelse(
  params$kuadrc_bmi | params$id == "TestJayhawk",
  "imperial",
  "metric"
)
...
anthro <- anthro %>%
  ...
  BRIDGE21:::calculate_bmi(weight, height, units = anthro_units)
...
anthroMaxIndex <- find_max_date(anthro)

ltex <- glue::glue(
  ltex,
  bmiLast = round(anthro[[anthroMaxIndex, "bmi"]], 1),
  .open = "{{",
  .close = "}}"
)
...
bmi_plot <- BRIDGE21::plot_body_mass_index(
  anthro[[anthroMaxIndex, "weight"]],
  anthro[[anthroMaxIndex, "height"]]
)
...
weight_plot <- BRIDGE21::plot_weight(
  anthro[[anthroMaxIndex, "weight"]],
  anthro[[anthroMaxIndex, "height"]]
)
...
anthro %>%
  ...
  dplyr::mutate(weight = round(weight * 2.205, 1)) %>%
  flextable::flextable() %>%
