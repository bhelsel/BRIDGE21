...
if (nrow(anthro) > 0) {
  weight_col <- if (params$kuadrc_bmi) "weight" else "average_weight"

  anthro %>%
    BRIDGE21::format_date(!!date) %>%
    dplyr::mutate(
      weight_lbs = if (params$bold_bmi) {
        round(!!rlang::sym(weight_col) * 2.205, 1)
      } else {
        round(!!rlang::sym(weight_col), 1)
      },
      bmi = round(bmi, 1)
    ) %>%
    dplyr::select(Date, weight_lbs, bmi) %>%
    flextable::flextable() %>%
    ...
    flextable::width(j = 1:3, width = 1.5) %>%
