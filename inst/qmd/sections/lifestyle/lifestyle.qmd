...
check_missing_wt_ht <- function(wt, ht, missing_vals = 88.8) {
  ht <- ifelse(ht %in% missing_vals, NA, ht)
  anthro[["height"]] <<- ht
  len <- c(wt = sum(!is.na(wt)), ht = sum(!is.na(ht)))
  if (len[["wt"]] > len[["ht"]] & len[["ht"]] == 1) {
    anthro[["height"]] <<- ht[!is.na(ht)]
    len[["ht"]] <- len["wt"]
  }
...
if (params$kuadrc_dxa | params$bold_dxa) {
  dxaVars <- c(
    ...
    "lean|dxa_lean_g",
    "fat|dxa_fat_g",
    "total_fat|dxa_fat_percent",
    "dxa_bmd_z"
  )

  dxa <- BRIDGE21::get_database_values(!!!dxaVars, date = date, data = data)

  dxa <- dxa[complete.cases(dxa[setdiff(names(dxa), date)]), ]
