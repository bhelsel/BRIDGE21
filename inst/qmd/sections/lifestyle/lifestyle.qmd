...
```{r controller to determine what lifestyle sections would have data for the bold study}
#| echo: false
#| results: asis
#| eval: !expr params$bold_lifestyle
...
if (rlang::is_empty(blood_pressure)) {
  params$bold_blood_pressure <- FALSE
}
...
missing_bmi <- (length(anthro$average_weight) == 0 | length(anthro$average_height) == 0) &
  length((anthro$average_weight) != length(anthro$average_height))

if (missing_bmi) {
  params$bold_bmi <- FALSE
}
