...
if (params$kuadrc_bmi) {
  anthro <- BRIDGE21::get_database_values(weight, height, data = data)
  ...
  missing_bmi <- (length(anthro$average_weight) == 0 |
    length(anthro$average_height) == 0) |
    length(anthro$average_weight) != length(anthro$average_height)
}
if (rlang::is_empty(bmi)) {
  params$kuadrc_bmi <- params$bold_bmi <- FALSE
}

if (params$kuadrc_dxa) {
  dxa <- BRIDGE21::get_database_values(
    fittest_date,
    mass,
    lean,
    fat,
