...
# params <- yaml::read_yaml("test_jayhawk/_variables.yaml")
# data <- readr::read_csv("test_jayhawk/data.csv", show_col_types = FALSE)
...
if ("birthsex" %in% colnames(data)) {
  data$sex <- ifelse(is.na(data$sex), data$birthsex, data$sex)
}

if ("frmdatea1" %in% colnames(data)) {
...
BRIDGE21::format_quarto(
  BRIDGE21::add_string(
    "Each time you are seen, we have a team of clinicians review " +
      ...
      "Below is the clinician’s feedback from your last visit. "
  ),
  type = "strwrap"
)
...
intake <- BRIDGE21::get_database_values(
  "subject_fname|firstname",
  "subject_lname|lastname,",
  "subject_birthdate|scrn_dob",
  sex,
  data = data,
  event_name = redcap_event_name
)
...
visit$age_at_visit <- floor(
  as.numeric(visit$visitdt - dob) / 365.25
)
