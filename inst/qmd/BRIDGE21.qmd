...
find_max_date <- function(data) {
  ...
  MaxIndex <- which(data[["Date"]] == max(data[["Date"]]))
  return(MaxIndex)
}

# params <- yaml::read_yaml("TestJayhawk/_variables.yaml")
# data <- readr::read_csv("TestJayhawk/data.csv", show_col_types = FALSE)
...
BRIDGE21::format_quarto(
  sprintf(
    "%s: This report is not meant to be handed to participants",
    params$id
  ),
  type = "text",
  color = "red",
...
#| echo: false
#| eval: !expr (any(params[grepl("kuadrc", names(params))] == TRUE)) | (any(params[grepl("bold", names(params))] == TRUE))
#| results: asis
